...
if (!empty($_POST["user_reg"])) {
  $sql = "SELECT * FROM bdr_users WHERE userid LIKE '%{$_POST["user_reg"]}%'";
  ...
  if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
      ...
      <tr>
        ...
        <td>{$row["fullname"]}</td>
        <td>{$row["gender"]}</td>
        <td><a href="view_usr.php?company={$_SESSION['comp_id']}&user={$row['id']}" class="btn btn-info btn-sm">View</a></td>
      </tr>
      ROW;
    }
  } else {
